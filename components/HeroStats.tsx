...
function FloatingStat({ stat, mouseX, mouseY, index }: { stat: ImpactStat; mouseX: MotionValue<number>; mouseY: MotionValue<number>; index: number }) {
  ...
  return <motion.div initial={{ opacity: 0, filter: "blur(12px)", scale: 0.82 }} animate={{ opacity: 1, filter: "blur(0px)", scale: 1 }} transition={{ delay: 0.35 + index * 0.12, duration: 1.1, ease: [0.16, 1, 0.3, 1] }} style={{ x, y }} className={`pointer-events-auto absolute hidden xl:block ${stat.desktopPosition}`}><motion.div animate={stat.float} transition={{ duration: stat.float.duration, repeat: Infinity, ease: "easeInOut" }} whileHover={{ rotate: 3, scale: 1.025, filter: "brightness(1.08)" }} className="hero-stat-sphere group h-32 w-32 p-3.5"><div className="hero-stat-reflection" /><div className="relative flex h-full flex-col items-center justify-center text-center"><span className="text-[1.65rem] font-bold tracking-[-0.08em] text-foreground"><AnimatedNumber value={stat.value} suffix={stat.displaySuffix} /></span><span className="mt-1 text-[11px] font-medium leading-tight text-foreground">{stat.label}</span><span className="mt-1.5 text-[8px] leading-snug text-muted"><OrganizationText>{stat.description}</OrganizationText></span></div></motion.div></motion.div>;
}
