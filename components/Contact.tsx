...
export default function Contact({ locale, content }: { locale: Locale; content?: { title: string; description: string } }) {
  ...
  return <section id="contact" onPointerMove={(event) => { if (activeSignal !== null) setPointer({ x: event.clientX, y: event.clientY }); }} onPointerLeave={() => { setActiveSignal(null); setPointer(null); }} className="relative overflow-hidden border-t border-border bg-[#0c0c0b] px-5 py-20 text-[#f5f5f1] sm:px-8 md:px-12 md:py-24">
    ...
    <div className="relative mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1.2fr_.8fr] lg:items-end lg:gap-24">
      <motion.div initial={{ opacity: 0, y: 34 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: .75, ease: [0.16, 1, .3, 1] }}>
        ...
        <h2 className="mt-6 max-w-[10ch] text-[clamp(3.5rem,6.5vw,7rem)] font-medium leading-[.84] tracking-[-.075em] text-white">{values.title}</h2>
        <p className="mt-7 max-w-xl text-base leading-relaxed text-white/65 sm:text-lg">{values.description}</p>
      </motion.div>
      <motion.div initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: .75, delay: .1, ease: [0.16, 1, .3, 1] }} className="border-t border-white/20">
        {actions.map(({ label, href, icon: Icon, external }, index) => <a key={label} href={href} target={external ? "_blank" : undefined} rel={external ? "noopener noreferrer" : undefined} onPointerEnter={(event) => { setActiveSignal(index as 0 | 1 | 2); setPointer({ x: event.clientX, y: event.clientY }); }} onFocus={() => setActiveSignal(index as 0 | 1 | 2)} onBlur={() => { setActiveSignal(null); setPointer(null); }} className="group flex items-center justify-between border-b border-white/20 py-5 text-lg font-medium tracking-[-.035em] text-white transition-colors hover:text-white/60"><span className="flex items-center gap-3"><Icon size={18} />{label}</span><ArrowUpRight className="transition-transform duration-300 group-hover:translate-x-1 group-hover:-translate-y-1" size={19} /></a>)}
      </motion.div>
    </div>
  </section>;
}
