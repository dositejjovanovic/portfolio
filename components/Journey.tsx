  ...
  return (
    <Section id="journey" eyebrow={copy.eyebrow} title={copy.title} description={copy.description}>
      <div className="grid gap-8 lg:grid-cols-[minmax(0,.88fr)_minmax(20rem,1.12fr)] lg:gap-16">
        ...
        {active && <AnimatePresence mode="wait"><motion.article key={`${active.period}-${active.title}`} initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: .38, ease: [0.16, 1, .3, 1] }} className="relative min-h-[20rem] overflow-hidden rounded-[2rem] border border-border bg-card p-6 shadow-[0_24px_72px_var(--shadow)] sm:p-7"><div className="absolute inset-x-0 top-0 h-1 bg-glow" /><div className="flex items-start justify-between gap-6"><p className="editorial-kicker text-glow">{active.period}</p>{active.logos?.length ? <div className="flex gap-2">{active.logos.map((logo) => <div key={logo.src} className="grid h-12 w-12 place-items-center rounded-2xl border border-border bg-background p-2"><Image src={logo.src} alt={logo.alt} width={48} height={48} unoptimized className="h-full w-full object-contain" /></div>)}</div> : null}</div><h3 className="mt-7 max-w-[14ch] text-[clamp(2rem,3.8vw,3.75rem)] font-medium leading-[.88] tracking-[-.065em] text-foreground">{active.title}</h3><p className="mt-4 max-w-xl text-sm leading-relaxed text-muted sm:text-base">{active.description}</p><div className="mt-6 flex flex-wrap gap-2">{active.responsibilities.map((item) => <span key={item} className="rounded-full border border-border bg-background px-3 py-1 text-[11px] text-muted">{item}</span>)}</div>{active.href && <Link href={localPath(locale, active.href)} className="mt-7 inline-flex items-center gap-2 text-sm font-semibold text-foreground underline decoration-glow decoration-2 underline-offset-4 hover:text-glow">{active.linkLabel ?? copy.view} <span aria-hidden>↗</span></Link>}</motion.article></AnimatePresence>}
      </div>
    </Section>
  );
}
