...
export async function readRepositoryBinaryFile(path: string) { const response = await request(path); const file = await response.json() as GitHubFile; return { sha: file.sha, content: Buffer.from(file.content.replace(/\n/g, ""), "base64") }; }
