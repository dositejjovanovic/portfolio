...
const homeContentSchema = z.object({
  ...
  international: z.array(z.object({ title: z.string().min(1).max(180), city: z.string().min(1).max(100), country: z.string().min(1).max(100), year: z.string().min(1).max(30), organization: z.string().max(180).optional(), role: z.string().max(180).optional(), description: z.string().min(1).max(1200), contribution: z.array(z.string().max(300)).optional(), relatedPost: slugSchema.optional(), featured: z.boolean().optional() })).max(40),
  awards: z.array(z.object({ field: z.enum(["Mathematics", "Physics", "Chemistry", "Film", "Academic distinctions"]), title: z.string().min(1).max(240), level: z.enum(["First Prize", "Second Prize", "Third Prize", "Special Award"]).optional(), year: z.string().max(20).optional(), note: z.string().max(500).optional() })).max(60),
